...
beforeEach(() => {
  ...
  (ocQueries.getObstacleTraining as any).mockResolvedValue({
    id,
    ocId,
    semester: 4,
    obstacle: 'High wall',
...
describe('PATCH /api/v1/oc/:ocId/obstacle-training/:id', () => {
  ...
  it('returns 400 for invalid UUID in path params', async () => { (ocChecks.mustBeAuthed as any).mockResolvedValueOnce({ userId: 'admin-1', roles: ['ADMIN'] }); (ocChecks.parseParam as any).mockRejectedValueOnce(new ApiError(400, 'Bad request', 'bad_request')); const req = makeJsonRequest({ method: 'PATCH', path: `${basePath}/not-a-uuid/obstacle-training/not-a-uuid`, body: { remark: 'Updated' } }); const ctx = { params: { ocId: 'not-a-uuid', id: 'not-a-uuid' } } as any; const res = await patchObstacleTrainingRoute(req as any, ctx); expect(res.status).toBe(400); const body = await res.json(); expect(body.ok).toBe(false); expect(body.error).toBe('bad_request'); });
  ...
  it('returns 404 when obstacle training record to update is not found', async () => { (ocChecks.mustBeAuthed as any).mockResolvedValueOnce({ userId: 'admin-1', roles: ['ADMIN'] }); (ocChecks.parseParam as any).mockResolvedValueOnce({ ocId }).mockResolvedValueOnce({ id }); (ocChecks.ensureOcExists as any).mockResolvedValueOnce(undefined); (ocQueries.updateObstacleTraining as any).mockResolvedValueOnce(null); const req = makeJsonRequest({ method: 'PATCH', path: `${basePath}/${ocId}/obstacle-training/${id}`, body: { remark: 'Updated' } }); const ctx = { params: { ocId, id } } as any; const res = await patchObstacleTrainingRoute(req as any, ctx); expect(res.status).toBe(404); const body = await res.json(); expect(body.ok).toBe(false); expect(body.error).toBe('not_found'); });

  it('returns 200 with updated obstacle training record on happy path', async () => { (ocChecks.mustBeAuthed as any).mockResolvedValueOnce({ userId: 'admin-1', roles: ['ADMIN'] }); (ocChecks.parseParam as any).mockResolvedValueOnce({ ocId }).mockResolvedValueOnce({ id }); (ocChecks.ensureOcExists as any).mockResolvedValueOnce(undefined); const updated = { id, ocId, semester: 4, obstacle: 'High wall', marksObtained: 19.0, remark: 'Updated' }; (ocQueries.updateObstacleTraining as any).mockResolvedValueOnce(updated); const req = makeJsonRequest({ method: 'PATCH', path: `${basePath}/${ocId}/obstacle-training/${id}`, body: { remark: 'Updated' } }); const ctx = { params: { ocId, id } } as any; const res = await patchObstacleTrainingRoute(req as any, ctx); expect(res.status).toBe(200); const body = await res.json(); expect(body.ok).toBe(true); expect(body.data.id).toBe(id); });
});
...
describe('DELETE /api/v1/oc/:ocId/obstacle-training/:id', () => {
  ...
  it('returns 400 for invalid UUID in path params', async () => { (ocChecks.mustBeAuthed as any).mockResolvedValueOnce({ userId: 'admin-1', roles: ['ADMIN'] }); (ocChecks.parseParam as any).mockRejectedValueOnce(new ApiError(400, 'Bad request', 'bad_request')); const req = makeJsonRequest({ method: 'DELETE', path: `${basePath}/not-a-uuid/obstacle-training/not-a-uuid` }); const ctx = { params: { ocId: 'not-a-uuid', id: 'not-a-uuid' } } as any; const res = await deleteObstacleTrainingRoute(req as any, ctx); expect(res.status).toBe(400); const body = await res.json(); expect(body.ok).toBe(false); expect(body.error).toBe('bad_request'); });
  ...
  it('returns 404 when obstacle training record to delete is not found', async () => { (ocChecks.mustBeAuthed as any).mockResolvedValueOnce({ userId: 'admin-1', roles: ['ADMIN'] }); (ocChecks.parseParam as any).mockResolvedValueOnce({ ocId }).mockResolvedValueOnce({ id }); (ocChecks.ensureOcExists as any).mockResolvedValueOnce(undefined); (ocQueries.deleteObstacleTraining as any).mockResolvedValueOnce(null); const req = makeJsonRequest({ method: 'DELETE', path: `${basePath}/${ocId}/obstacle-training/${id}` }); const ctx = { params: { ocId, id } } as any; const res = await deleteObstacleTrainingRoute(req as any, ctx); expect(res.status).toBe(404); const body = await res.json(); expect(body.ok).toBe(false); expect(body.error).toBe('not_found'); });
  ...
  it('returns 403 when hard delete lacks admin privileges', async () => { (ocChecks.mustBeAdmin as any).mockRejectedValueOnce(new ApiError(403, 'Forbidden', 'forbidden')); const req = makeJsonRequest({ method: 'DELETE', path: `${basePath}/${ocId}/obstacle-training/${id}?hard=true` }); const ctx = { params: { ocId, id } } as any; const res = await deleteObstacleTrainingRoute(req as any, ctx); expect(res.status).toBe(403); const body = await res.json(); expect(body.ok).toBe(false); expect(body.error).toBe('forbidden'); });
