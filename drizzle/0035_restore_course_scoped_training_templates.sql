ALTER TABLE "pt_types" ADD COLUMN IF NOT EXISTS "course_id" uuid;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'pt_types_course_id_courses_id_fk'
          AND conrelid = 'public.pt_types'::regclass
    ) THEN
        ALTER TABLE "pt_types"
        ADD CONSTRAINT "pt_types_course_id_courses_id_fk"
        FOREIGN KEY ("course_id") REFERENCES "public"."courses"("id") ON DELETE restrict ON UPDATE no action;
    END IF;
END $$;
--> statement-breakpoint
ALTER TABLE "pt_motivation_award_fields" ADD COLUMN IF NOT EXISTS "course_id" uuid;
--> statement-breakpoint
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'pt_motivation_award_fields_course_id_courses_id_fk'
          AND conrelid = 'public.pt_motivation_award_fields'::regclass
    ) THEN
        ALTER TABLE "pt_motivation_award_fields"
        ADD CONSTRAINT "pt_motivation_award_fields_course_id_courses_id_fk"
        FOREIGN KEY ("course_id") REFERENCES "public"."courses"("id") ON DELETE restrict ON UPDATE no action;
    END IF;
END $$;
--> statement-breakpoint
ALTER TABLE "training_camps" ADD COLUMN IF NOT EXISTS "course_id" uuid;
--> statement-breakpoint
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'training_camps_course_id_courses_id_fk'
          AND conrelid = 'public.training_camps'::regclass
    ) THEN
        ALTER TABLE "training_camps"
        ADD CONSTRAINT "training_camps_course_id_courses_id_fk"
        FOREIGN KEY ("course_id") REFERENCES "public"."courses"("id") ON DELETE restrict ON UPDATE no action;
    END IF;
END $$;
--> statement-breakpoint
ALTER TABLE "interview_templates" ADD COLUMN IF NOT EXISTS "course_id" uuid;
--> statement-breakpoint
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'interview_templates_course_id_courses_id_fk'
          AND conrelid = 'public.interview_templates'::regclass
    ) THEN
        ALTER TABLE "interview_templates"
        ADD CONSTRAINT "interview_templates_course_id_courses_id_fk"
        FOREIGN KEY ("course_id") REFERENCES "public"."courses"("id") ON DELETE restrict ON UPDATE no action;
    END IF;
END $$;
--> statement-breakpoint

DROP INDEX IF EXISTS "uq_pt_type_semester_code";
--> statement-breakpoint
DROP INDEX IF EXISTS "uq_pt_motivation_field_semester";
--> statement-breakpoint
DROP INDEX IF EXISTS "uq_training_camp_name_semester";
--> statement-breakpoint
DROP INDEX IF EXISTS "idx_training_camp_semester_sort_name";
--> statement-breakpoint
DROP INDEX IF EXISTS "uq_interview_template_code";
--> statement-breakpoint

CREATE UNIQUE INDEX IF NOT EXISTS "uq_pt_type_course_semester_code" ON "pt_types" USING btree ("course_id","semester","code");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "idx_pt_type_course_semester_sort" ON "pt_types" USING btree ("course_id","semester","sort_order");
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS "uq_pt_motivation_field_course_semester" ON "pt_motivation_award_fields" USING btree ("course_id","semester","label");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "idx_pt_motivation_field_course_semester_sort" ON "pt_motivation_award_fields" USING btree ("course_id","semester","sort_order");
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS "uq_training_camp_name_course_semester" ON "training_camps" USING btree ("course_id","name","semester");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "idx_training_camp_course_semester_sort_name" ON "training_camps" USING btree ("course_id","semester","sort_order","name");
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS "uq_interview_template_course_code" ON "interview_templates" USING btree ("course_id","code");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "idx_interview_template_course_sort" ON "interview_templates" USING btree ("course_id","sort_order");
--> statement-breakpoint
...
WITH rows_to_clone AS (
    ...
    WHERE NOT EXISTS (
        SELECT 1
        FROM pt_type_attempts existing
        WHERE existing.pt_type_id = m.target_type_id
...
WITH rows_to_clone AS (
    ...
    WHERE NOT EXISTS (
        SELECT 1
        FROM pt_attempt_grades existing
        WHERE existing.pt_attempt_id = m.target_attempt_id
...
INNER JOIN tmp_pt_grade_map grade_map
   ...
   AND grade_map.course_id = task_map.course_id
WHERE NOT EXISTS (
    SELECT 1
    FROM pt_task_scores existing
    WHERE existing.pt_task_id = task_map.target_task_id
      AND existing.pt_attempt_id = attempt_map.target_attempt_id
      AND existing.pt_attempt_grade_id = grade_map.target_grade_id
);
--> statement-breakpoint
...
INNER JOIN tmp_training_camp_map camp_map ON camp_map.source_camp_id = activity.training_camp_id
WHERE NOT EXISTS (
    SELECT 1
    FROM training_camp_activities existing
    WHERE existing.training_camp_id = camp_map.target_camp_id
      AND existing.name = activity.name
      AND existing.sort_order = activity.sort_order
);
--> statement-breakpoint
...
INNER JOIN tmp_interview_template_map map ON map.source_template_id = sem.template_id
WHERE NOT EXISTS (
    SELECT 1
    FROM interview_template_semesters existing
    WHERE existing.template_id = map.target_template_id
      AND existing.semester = sem.semester
);
--> statement-breakpoint
...
WITH rows_to_clone AS (
    ...
    WHERE NOT EXISTS (
        SELECT 1
        FROM interview_template_sections existing
        WHERE existing.template_id = map.target_template_id
          AND existing.title = section.title
          AND existing.sort_order = section.sort_order
    )
)
...
INNER JOIN interview_template_sections cloned
   ...
   AND cloned.sort_order = section.sort_order;
--> statement-breakpoint
...
WITH rows_to_clone AS (
    ...
    WHERE NOT EXISTS (
        SELECT 1
        FROM interview_template_groups existing
        WHERE existing.template_id = map.target_template_id
          AND existing.section_id IS NOT DISTINCT FROM section_map.target_section_id
          AND existing.title = grp.title
          AND existing.sort_order = grp.sort_order
    )
)
...
INNER JOIN interview_template_groups cloned
   ...
   AND cloned.sort_order = grp.sort_order;
--> statement-breakpoint
...
WITH rows_to_clone AS (
    ...
    WHERE NOT EXISTS (
        SELECT 1
        FROM interview_template_fields existing
        WHERE existing.template_id = map.target_template_id
...
INNER JOIN tmp_interview_field_map field_map ON field_map.source_field_id = opt.field_id
WHERE NOT EXISTS (
    SELECT 1
    FROM interview_template_field_options existing
    WHERE existing.field_id = field_map.target_field_id
