...
async function assertCadetInPlatoon(
  ...
  if (!cadet) {
    throw new ApiError(
      400,
      "Selected cadet is not active in your platoon.",
      "bad_request"
    );
  }
}
...
async function selectAppointmentById(
  tx: TxLike,
  appointmentId: string,
  platoonId: string
) {
  const [row] = await tx
    ...
    .where(
      and(
        eq(cadetAppointments.id, appointmentId),
...
async function listCadetAppointments(
  ...
  return db
    ...
    .orderBy(
      kind === "active"
        ? asc(cadetAppointments.appointmentName)
        : desc(cadetAppointments.endsAt)
    );
}
...
export async function getCadetAppointmentsDashboard(
  ...
  const [platoon, cadets, activeAppointments, historyAppointments] = await Promise.all([
    ...
    db
      ...
      .from(ocCadets)
      .where(
        and(eq(ocCadets.platoonId, platoonId), eq(ocCadets.status, "ACTIVE"))
      )
...
export async function updateCadetAppointment(params: {
  ...
  const afterRowId = await db.transaction(async (tx) => {
    ...
    const nextEndsAt =
      params.endsAt !== undefined ? params.endsAt : current.endsAt;
  ...
  const dashboard = await getCadetAppointmentsDashboard(params.platoonId);
  const after =
    dashboard.activeAppointments.find((item) => item.id === afterRowId.id) ??
    dashboard.historyAppointments.find((item) => item.id === afterRowId.id);
...
export async function transferCadetAppointment(params: {
  ...
  const txResult = await db.transaction(async (tx) => {
    ...
    if (current.endsAt && current.endsAt <= new Date()) {
      throw new ApiError(
        409,
        "Cannot transfer a completed cadet appointment.",
        "conflict"
      );
    }
    ...
    if (params.newStartsAt <= params.prevEndsAt) {
      throw new ApiError(
        400,
        "Takeover must be after the handover date.",
        "bad_request"
      );
    }
