...
export async function resolveTrainingCampTemplateCopySource(
    sourceCourseId: string,
    semester: number,
    loadCamps: typeof listTrainingCamps = listTrainingCamps,
) {
    const courseCamps = await loadCamps({
    ...
    if (courseCamps.length > 0) {
        return courseCamps;
    }

    const defaultCamps = await loadCamps({
        courseId: null,
        semester,
        includeActivities: true,
        includeDeleted: false,
...
export async function copyTrainingCampTemplateSemester(input: {
    ...
    const sourceCamps = await resolveTrainingCampTemplateCopySource(input.sourceCourseId, input.semester);
