...
export default function WarningManagementPage() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const response = await warningManagementApi.updateSettings({ criteria: draft });
      setDraft(buildWarningCriteriaForActiveAppointments(
        response.criteria,
        Array.from(appointmentPositions.values()),
      ));
      toast.success("Warning management settings saved.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to save warning settings.");
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      <main className="space-y-6 p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <Button onClick={handleSave} disabled={saving || settingsQuery.isLoading || appointmentsQuery.isLoading}>
