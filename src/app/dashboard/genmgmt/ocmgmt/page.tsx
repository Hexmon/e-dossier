...
type BulkResult = { success: number; failed: number; errors: Array<{ row: number; error: string }>; } | null;
...
export default function OCManagementPage() {
  ...
  // fetch OCs when filters/search change
  useEffect(() => {
    (async () => {
      const items = await fetchOCs({
      ...
      setOcList(items || []);
    })().catch((e) => console.error("fetch OCs failed:", e));
  }, [debouncedSearch, courseFilter, platoonFilter, statusFilter, branchFilter]);

  const onSubmit: SubmitHandler<Partial<OCRecord>> = useCallback(async (data) => {
    try {
      if (editIndex !== null) {
  ...
  const doBulkUpload = useCallback(async () => {
    ...
    try {
      ...
      const items = await fetchOCs({ q: debouncedSearch || undefined });
      setOcList(items || []);
    } catch (err) {
      console.error(err);
      setBulkResult({
  ...
  }, [uploadedRawRows, debouncedSearch]);
  ...
  const doValidateRow = useCallback(async (rowIndex: number) => {
    const row = uploadedRawRows[rowIndex];
    if (!row) return;
    try {
      const res = await bulkValidateOCs([row]);
      setRowValidations((prev) => ({ ...prev, [rowIndex]: res.failed > 0 ? res.errors[0]?.error ?? "error" : "ok" }));
    } catch (e: any) {
    ...
    <SidebarProvider>
      <section className="min-h-screen flex w-full bg-background">
        ...
        <main className="flex-1 flex flex-col">
          <header className="h-16 border-b border-border bg-card/50 backdrop-blur sticky top-0 z-50">
            <PageHeader title="OC Management" description="Manage all OC details across platoons and terms" onLogout={() => { }} />
          </header>

          <section className="flex-1 p-6">
            <nav>
              <BreadcrumbNav paths={[{ label: "Dashboard", href: "/dashboard" }, { label: "Gen Mgmt", href: "/dashboard/genmgmt" }, { label: "OC Management" }]} />
            </nav>
              ...
              <TabsContent value="oc-mgmt" className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" onClick={handleAddOC}>Add OC</Button>
                    <UploadButton disabled={parsing} onParsed={onParsed} onParsingStateChange={setParsing} label={parsing ? "Parsing…" : "Upload CSV / Excel"} />
                  </div>
                </div>

                {/* Filters */}
                <div className="grid grid-cols-5 gap-3">
                  <Input placeholder="Search name / TES no…" value={search} onChange={(e) => setSearch(e.target.value)} />
                  <select className="border rounded px-2 py-2" value={courseFilter} onChange={(e) => setCourseFilter(e.target.value)}>
                    <option value="">All Courses</option>
                    {courses.map((c) => (<option key={c.id} value={c.id}>{c.code ?? c.title ?? c.id}</option>))}
                  </select>
                  <select className="border rounded px-2 py-2" value={platoonFilter} onChange={(e) => setPlatoonFilter(e.target.value)}>
                    <option value="">All Platoons</option>
                    {platoons.map((p) => (<option key={p.id} value={p.id}>{p.name ?? p.id}</option>))}
                  </select>
                  <select className="border rounded px-2 py-2" value={branchFilter} onChange={(e) => setBranchFilter(e.target.value)}>
                ...
                <UploadPreviewTable
                  ...
                  onClear={() => { setUploadedPreview([]); setUploadedRawRows([]); setBulkResult(null); setValidateResult(null); setRowValidations({}); }}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editIndex !== null ? "Update OC" : "Add New OC"}</DialogTitle></DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div><Label>Tes No</Label><Input {...register("ocNo" as any, { required: true })} /></div>
            <div>
              <Label>Course</Label>
              <select {...register("courseId" as any, { required: true })} className="w-full border border-input bg-background rounded-md p-2" defaultValue="">
                <option value="" disabled>Select Course</option>
                {courses.map((c) => (<option key={c.id} value={c.id}>{c.code ?? c.title ?? c.id}</option>))}
              </select>
            </div>
            <div><Label>Branch</Label><Input {...register("branch" as any)} placeholder="E / M / O" /></div>
            <div>
              <Label>Platoon</Label>
              <select {...register("platoonId" as any)} className="w-full border border-input bg-background rounded-md p-2" defaultValue="">
                <option value="" disabled>Select Platoon</option>
                {platoons.map((p) => (<option key={p.id} value={p.id}>{p.name ?? p.id}</option>))}
              </select>
            </div>
            <div><Label>Arrival Date</Label><Input type="date" {...register("arrivalAtUniversity" as any)} /></div>
