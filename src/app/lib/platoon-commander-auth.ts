...
export async function requirePlatoonCommanderScope(
  ...
  const position =
    typeof authCtx.claims?.apt?.position === "string" ? authCtx.claims.apt.position : null;
  const scopeType =
    typeof authCtx.claims?.apt?.scope?.type === "string"
      ? authCtx.claims.apt.scope.type
      : null;
  const scopeId =
    typeof authCtx.claims?.apt?.scope?.id === "string" ? authCtx.claims.apt.scope.id : null;
  ...
  if (
    ...
    throw new ApiError(403, "Forbidden: platoon commander access required.", "forbidden");
  }
