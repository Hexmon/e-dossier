...
async function existsOcNo(ocNo: string): Promise<boolean> {
  const rows = await db.select({ id: ocCadets.id }).from(ocCadets).where(eq(ocCadets.ocNo, ocNo)).limit(1);
  return !!rows[0];
}
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        console.error('[oc/bulk-upload] failed row', i + 1, e);
        // In case DB unique constraints exist, show friendly messages
        if (e?.code === '23505') {
          // crude hinting by constraint names if you have them; otherwise generic:
