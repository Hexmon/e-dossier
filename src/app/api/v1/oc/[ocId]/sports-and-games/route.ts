...
async function GETHandler(req: AuditNextRequest, { params }: { params: Promise<{ ocId: string }> }) {
  try {
    ...
    await req.audit.log({
      ...
      metadata: {
        description: 'Sports and games records retrieved successfully.',
        ocId,
    ...
    return json.ok({ message: 'Sports and games records retrieved successfully.', items: rows, count: rows.length });
  } catch (err) {
...
async function POSTHandler(req: AuditNextRequest, { params }: { params: Promise<{ ocId: string }> }) {
  try {
    ...
    await req.audit.log({
      ...
      target: { type: AuditResourceType.OC, id: ocId },
      metadata: {
        description: `Created sports/games record ${row.id} for OC ${ocId}`,
        ocId,
        module: 'sports_and_games',
        recordId: row.id,
      },
    ...
    return json.created({ message: 'Sports and games record created successfully.', data: row });
  } catch (err) {
    return handleApiError(err);
  }
}
