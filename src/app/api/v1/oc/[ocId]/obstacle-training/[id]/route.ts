...
async function GETHandler(req: AuditNextRequest, { params }: { params: Promise<{ ocId: string }> }) {
  try {
    ...
    await req.audit.log({
      ...
      metadata: {
        description: `Obstacle training record ${row.id} retrieved successfully.`,
        ocId,
...
async function PATCHHandler(req: AuditNextRequest, { params }: { params: Promise<{ ocId: string }> }) {
  try {
    ...
    const previous = await getObstacleTraining(ocId, id);
    if (!previous) throw new ApiError(404, 'Obstacle training record not found', 'not_found');
