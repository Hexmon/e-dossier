...
import { hardDeleteSiteCommander } from "@/app/db/queries/site-settings";
...
async function DELETEHandler(
  ...
  try {
    ...
    const result = await hardDeleteSiteCommander(id);
    if (!result) {
    ...
    await req.audit.log({
      ...
      target: { type: AuditResourceType.API, id: "admin:site-settings:commanders:hard-delete" },
      metadata: {
        eventType: "SITE_COMMANDER_HARD_DELETED",
        description: "Commander entry hard deleted.",
        commanderId: id,
      },
    });

    return json.ok({ message: "Commander hard-deleted successfully.", id });
  } catch (error) {
