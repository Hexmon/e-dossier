...
import { getPtType, listPtAttempts, createPtAttempt, findPtAttemptByTypeAndSortOrder, getNextPtAttemptSortOrder } from '@/app/db/queries/physicalTraining';
...
async function POSTHandler(req: AuditNextRequest, { params }: { params: Promise<{ typeId: string }> }) {
    try {
        ...
        const targetSortOrder = dto.sortOrder ?? (await getNextPtAttemptSortOrder(typeId));
        const duplicate = await findPtAttemptByTypeAndSortOrder(typeId, targetSortOrder);
        if (duplicate) {
            throw new ApiError(
                409,
