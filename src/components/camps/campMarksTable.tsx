...
import { OcCamp } from "@/app/lib/api/ocCampApi";
  ...
  useEffect(() => {
    ...
    if (!campFormRow || formActivities.length === 0) {
      ...
      if (prefillData.length > 0) {
        // Check if we have saved scores from backend for this camp
        if (camp?.activityScores && camp.activityScores.length > 0) {
          // Map saved scores to the hardcoded activities structure
          initialActivities = prefillData.map((prefillItem) => {
            // Try to find a matching saved score by activity name
            const savedScore = camp.activityScores?.find(
              (score) => score.activity?.name === prefillItem.activity
            );

            return {
              trainingCampActivityId: savedScore?.trainingCampActivityId || prefillItem.id,
              name: prefillItem.activity,
              marksScored: savedScore ? Number(savedScore.marksScored) : null,
              defaultMaxMarks: prefillItem.maxMarks,
              remark: savedScore?.remark || null,
            };
          });
        } else {
          // No saved data, use fresh hardcoded structure
