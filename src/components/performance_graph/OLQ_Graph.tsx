// components/performance_graph/OlqChart.tsx
"use client";
...
export default function OlqChart({ data: olqData }: { data: number[] }) {
  ...
  const [averageData] = useState(computeAverageMarks("olq"));

  useEffect(() => {
    if (!Array.isArray(olqData) || olqData.length === 0) return;
    const highest = Math.max(...olqData);
    const average = olqData.reduce((a, b) => a + b, 0) / olqData.length;
    const lowest = Math.min(...olqData);
    const highestIndex = olqData.indexOf(highest);
      ...
      options: {
          ...
          tooltip: {
            ...
            callbacks: {
              label: function (context: any) {
                return `Score: ${context.parsed.y}/300`;
              },
            },
          },
        },
        scales: {
          ...
          y: {
            beginAtZero: true,
            max: 300,
            ticks: { stepSize: 75, font: { size: 11, weight: "500" as any }, color: "rgb(75,85,99)", padding: 8 },
    ...
    <div className="w-full max-w-4xl mx-auto p-6">
      <div className="bg-linear-to-br from-white to-slate-50 rounded-2xl shadow-lg border border-slate-100 overflow-hidden">
        ...
        <div className="bg-linear-to-r from-indigo-600 to-indigo-700 px-6 py-6">
          <h2 className="text-2xl font-bold text-white tracking-tight">🎯 OLQ</h2>
          <p className="text-indigo-100 text-sm mt-1">Officer Like Qualities Performance Tracking</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 px-6 pb-6">
          <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-100">
            ...
            <p className="text-2xl font-bold text-indigo-600 mt-1">{stats.highest}/300</p>
          ...
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
            ...
            <p className="text-2xl font-bold text-blue-600 mt-1">{stats.lowest}/300</p>
