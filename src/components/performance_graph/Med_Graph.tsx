...
export default function MedGraph({
  ...
  useEffect(() => {
    if (!Array.isArray(medicalData) || medicalData.length === 0) return;
    const available = medicalData
  ...
  useEffect(() => {
    ...
    const maxValue = Math.max(...medicalData, ...averageData, 0);
    const yMax = Math.max(6, Math.ceil(maxValue / 2) * 2);
    ...
    chartInstance.current = new ChartJS(ctx as any, {
      ...
      data: {
        ...
        datasets: [
          ...
          {
            label: "Course Average missed days",
            data: averageData,
  ...
  }, [medicalData, averageData, themeVersion]);
      ...
      <div className="bg-card rounded-2xl shadow-lg border border-border overflow-hidden">
        ...
        <div className="grid grid-cols-3 gap-4 px-6 pb-6">
          <div className="bg-primary/10 rounded-lg p-4 border border-primary/20">
            ...
            <p className="text-2xl font-bold text-primary mt-1">{stats.highest}</p>
