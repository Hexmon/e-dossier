...
const Marquee = ({ data, speed = 15, className }: MarqueeProps) => {
  // Create a much longer duplicated array for smoother infinite loop
  const extendedData = Array.from({ length: 4 }, () => data).flat();

  return (
    <div 
      className={cn(
        "relative overflow-hidden bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 py-3 w-full",
        className
      )}
    >
      ...
      >
        ...
        {extendedData.map((item, index) => (
          <span
            key={index}
            className="text-sm font-semibold text-white inline-block shrink-0"
          >
            {item}
          </span>
        ))}
